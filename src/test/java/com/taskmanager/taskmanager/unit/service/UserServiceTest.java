...
//                .email("example@example.com")
//                .password("password")
//                .role(new HashSet<>()) // ensures role set is initialized
//                .build();
//        user.setId(userId);
//    }
//
...
//
//    @Test
//    void save_ShouldAddDefaultRoleAndSave() {
//        when(userRepository.existsByEmailAndDeletedAtNull(user.getEmail())).thenReturn(false);
//        when(userRepository.save(any(User.class))).thenAnswer(i -> i.getArgument(0));
//
//        UserResponse response = userService.save(user);
//
//        assertTrue(response.authorities().contains(Role.TEAM_MEMBER));
...
//    @Test
//    void addRole_ShouldNotDuplicateRole() {
//        user.getRole().add(Role.TEAM_MEMBER);
//        when(userRepository.findByIdAndDeletedAtNull(userId)).thenReturn(Optional.of(user));
//
//        UserResponse response = userService.addRole(userId, Role.TEAM_MEMBER);
//
...
class UserServiceTest {
    ...
    void save_ShouldAddDefaultRoleAndSave() {
        ...
        assertTrue(response.authorities().contains(Role.TEAM_MEMBER.getDisplayName()));
        verify(userRepository).save(any(User.class));
    }
    ...
    void addRole_ShouldAddRoleSuccessfully() {
        ...
        assertTrue(response.authorities().contains(Role.TEAM_LEADER.getDisplayName()));
    }
    ...
    void removeRole_ShouldRemoveRole() {
        ...
        assertFalse(response.authorities().contains(Role.TEAM_LEADER.getDisplayName()));
    }
